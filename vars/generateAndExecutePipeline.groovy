...
def _assertMajorVersionMatchesReleaseVersion(majorVersion) {
    ...
    releaseBranchName = env.BRANCH_NAME.split('/')[-1]

    if (majorVersion.toString() != releaseBranchName) {

        error("Major version in application definition, ${majorVersion}, does not match release branch name: ${releaseBranchName}")
    }
}
